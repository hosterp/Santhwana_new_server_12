...
class GeneralUnpaidWizard(models.TransientModel):
    _name = 'general.unpaid.wizard'
    _description = 'General Unpaid Bills Report Wizard'
    ...
    def action_generate_pdf(self):
        ...
        return self.env.ref('homeo_doctor.general_unpaid_pdf_report_action').report_action(self)


# 2. Create Report Model to Handle Data
class GeneralUnpaidReport(models.AbstractModel):
    _name = 'report.homeo_doctor.general_unpaid_pdf_report'
    _description = 'General Unpaid Bills Report'

    @api.model
    def _get_report_values(self, docids, data=None):
        # Get the wizard record
        wizard = self.env['general.unpaid.wizard'].browse(docids)
