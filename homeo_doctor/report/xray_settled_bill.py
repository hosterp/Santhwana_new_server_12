...
class XRayPaidWizard(models.TransientModel):
    _name = 'xray.paid.wizard'
    _description = 'X Ray paid Bills Report Wizard'
    ...
    def action_generate_pdf(self):
        ...
        bills = self.env['xray.billing'].search([
        ...
        return self.env.ref('homeo_doctor.xray_paid_pdf_report_action').report_action(self)


# 2. Create Report Model to Handle Data
class XRayPaidReport(models.AbstractModel):
    _name = 'report.homeo_doctor.xray_paid_pdf_report'
    _description = 'X Ray Paid Bills Report'

    @api.model
    def _get_report_values(self, docids, data=None):
        # Get the wizard record
        wizard = self.env['xray.paid.wizard'].browse(docids)
        ...
        bills = self.env['xray.billing'].search(domain, order='bill_date asc')
