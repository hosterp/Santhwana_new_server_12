...
class GeneralPaidWizard(models.TransientModel):
    _name = 'general.paid.wizard'
    _description = 'General paid Bills Report Wizard'
    ...
    def action_generate_pdf(self):
        ...
        bills = self.env['general.billing'].search([
        ...
        return self.env.ref('homeo_doctor.general_paid_pdf_report_action').report_action(self)


# 2. Create Report Model to Handle Data
class GeneralPaidReport(models.AbstractModel):
    _name = 'report.homeo_doctor.general_paid_pdf_report'
    _description = 'General paid Bills Report'

    @api.model
    def _get_report_values(self, docids, data=None):
        # Get the wizard record
        wizard = self.env['general.paid.wizard'].browse(docids)
        ...
        bills = self.env['general.billing'].search(domain, order='bill_date asc')
