...
class AudiologyUnpaidWizard(models.TransientModel):
    _name = 'audiology.unpaid.wizard'
    _description = 'Audiology Unpaid Bills Report Wizard'
    ...
    def action_generate_pdf(self):
        ...
        return self.env.ref('homeo_doctor.audiology_unpaid_pdf_report_action').report_action(self)


# 2. Create Report Model to Handle Data
class AudiologyUnpaidReport(models.AbstractModel):
    _name = 'report.homeo_doctor.audiology_unpaid_pdf_report'
    _description = 'Audiology Unpaid Bills Report'

    @api.model
    def _get_report_values(self, docids, data=None):
        # Get the wizard record
        wizard = self.env['audiology.unpaid.wizard'].browse(docids)
        ...
        return {
            'doc_ids': docids,
            'doc_model': 'audiology.unpaid.wizard',
            'docs': wizard,
            'data': {
                'bills': bill_data,
