...
class XRayUnpaidWizard(models.TransientModel):
    _name = 'xray.unpaid.wizard'
    _description = 'X Ray Unpaid Bills Report Wizard'
    ...
    def action_generate_pdf(self):
        ...
        return self.env.ref('homeo_doctor.xray_unpaid_pdf_report_action').report_action(self)


# 2. Create Report Model to Handle Data
class XRayUnpaidReport(models.AbstractModel):
    _name = 'report.homeo_doctor.xray_unpaid_pdf_report'
    _description = 'X Ray Unpaid Bills Report'

    @api.model
    def _get_report_values(self, docids, data=None):
        # Get the wizard record
        wizard = self.env['xray.unpaid.wizard'].browse(docids)
