...
class CasualtyUnpaidWizard(models.TransientModel):
    _name = 'casualty.unpaid.wizard'
    _description = 'Casualty Unpaid Bills Report Wizard'
    ...
    def action_generate_pdf(self):
        ...
        return self.env.ref('homeo_doctor.casualty_unpaid_pdf_report_action').report_action(self)


# 2. Create Report Model to Handle Data
class CasualtyUnpaidReport(models.AbstractModel):
    _name = 'report.homeo_doctor.casualty_unpaid_pdf_report'
    _description = 'Casualty Unpaid Bills Report'

    @api.model
    def _get_report_values(self, docids, data=None):
        # Get the wizard record
        wizard = self.env['casualty.unpaid.wizard'].browse(docids)
        ...
        return {
            'doc_ids': docids,
            'doc_model': 'casualty.unpaid.wizard',
            'docs': wizard,
            'data': {
                'bills': bill_data,
