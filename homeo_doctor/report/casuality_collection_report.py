...
class CasualityCollectionReportWizard(models.TransientModel):
    _name = "casuality.collection.report.wizard"
    _description = "Casuality Collection Report Wizard"
    ...
    report_line_ids = fields.One2many(
        'casualty.billing.line.wizard', 'wizard_id', string="Report Lines"
    )
    ...
    def action_print_report(self):
        ...
        return self.env.ref('homeo_doctor.action_casuality_collection_report').report_action(self, data=data)
    ...
    def _get_doctor_collection(self, data):
        ...
        records = self.env['casuality.billing'].search(domain)
    ...
    def _prepare_report_lines(self):
        ...
        # Add new lines from casuality.billing
        domain = [('bill_date', '>=', self.date_from), ('bill_date', '<=', self.date_to), ('status', '!=', 'cancelled')]
        
        bills = self.env['casuality.billing'].search(domain)
        if self.doctor_id:
            bills = bills.filtered(lambda b: b.doctor.id == self.doctor_id.id)

        for bill in bills:
            for line in bill.general_bill_line_ids:
                bill_name = line.particulars.particular_name if line.particulars else ''
    ...
    def action_show_report(self):
        ...
        return {
            ...
            'name': 'Casualty Collection Report',
            'res_model': 'casualty.billing.line.wizard',
            'view_mode': 'tree,form',
            'views': [(self.env.ref('homeo_doctor.view_casualty_billing_line_tree').id, 'tree')],
            # 'search_view_id': self.env.ref('homeo_doctor.view_ot_billing_line_search').id,
            'target': 'current',
            'context': {
...
#
# class ReportCasualityCollection(models.AbstractModel):
#     _name = 'report.homeo_doctor.casuality_collection_report_template'
#     _description = 'Casuality Collection Report'
#
...
#         if doctor_id:
#             domain.append(('doctor', '=', doctor_id))
#
#         records = self.env['casuality.billing'].search(domain)
#
#         summary = {}
...
class ReportDoctorCollection(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        records = self.env['casuality.billing'].search(domain, order="bill_date asc")
        if doctor_id:
        ...
        if doctor_id:
            ...
            for rec in records:
                for line in rec.general_bill_line_ids:
                    bill_key = line.particulars.particular_name or "Unknown Bill"
...
class CasualtyBillingLineWizard(models.TransientModel):
    _name = "casualty.billing.line.wizard"
    _description = "casualty Billing Line Wizard"

    wizard_id = fields.Many2one('casuality.collection.report.wizard', string="Wizard")
    ...
    def unlink_line(self):
        ...
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'casuality.collection.report.wizard',
