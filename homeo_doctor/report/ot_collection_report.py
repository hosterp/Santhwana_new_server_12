...
class OtCollectionReportWizard(models.TransientModel):
    _name = "ot.collection.report.wizard"
    _description = "OT Collection Report Wizard"
    ...
    report_line_ids = fields.One2many(
        'ot.billing.line.wizard', 'wizard_id', string="Report Lines"
    )
    ...
    def action_print_report(self):
        ...
        return self.env.ref('homeo_doctor.action_ot_collection_report').report_action(self, data=data)
    ...
    def action_show_report(self):
        ...
        return {
            ...
            'name': 'OT Collection Report',
            'res_model': 'ot.billing.line.wizard',
            'view_mode': 'tree,form',
            'views': [(self.env.ref('homeo_doctor.view_ot_billing_line_tree').id, 'tree')],
...
#     _name = 'report.homeo_doctor.ot_collection_report_template'
#     _description = 'OT Collection Report'
#
...
#         if doctor_id:
#             domain.append(('doctor', '=', doctor_id))  # 👈 doctor filter applied here
#
#         records = self.env['ot.billing'].search(domain)
#
#         summary = {}
#         for rec in records:
...
#             'doctor_name': self.env['doctor.profile'].browse(doctor_id).name if doctor_id else "All Doctors",
#             'doctors': doctors,
#         }
...
class ReportDoctorCollection(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        records = self.env['ot.billing'].search(domain, order="bill_date asc")
        if doctor_id:
...
class OtBillingLineWizard(models.TransientModel):
    _name = "ot.billing.line.wizard"
    _description = "OT Billing Line Wizard"

    wizard_id = fields.Many2one('ot.collection.report.wizard', string="Wizard")
    ...
    def unlink_line(self):
        ...
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'ot.collection.report.wizard',
