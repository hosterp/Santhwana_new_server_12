...
class XRAYCollectionReportWizard(models.TransientModel):
    _name = "xray.collection.report.wizard"
    ...
    report_line_ids = fields.One2many(
        'xray.billing.line.wizard', 'wizard_id', string="Report Lines"
    )
    def action_print_report(self):
        ...
        return self.env.ref('homeo_doctor.action_xray_collection_report').report_action(self, data=data)
    ...
    def _get_doctor_collection(self, data):
        ...
        records = self.env['xray.billing'].search(domain)
    ...
    def _prepare_report_lines(self):
        ...
        # Add new lines from xray.billing
        domain = [('bill_date', '>=', self.date_from), ('bill_date', '<=', self.date_to), ('status', '!=', 'cancelled')]
        
        bills = self.env['xray.billing'].search(domain)
        if self.doctor_id:
    ...
    def action_show_report(self):
        ...
        return {
            ...
            'name': 'X-Ray Collection Report',
            'res_model': 'xray.billing.line.wizard',
            'view_mode': 'tree,form',
            'views': [(self.env.ref('homeo_doctor.view_xray_billing_line_tree').id, 'tree')],
            'search_view_id': self.env.ref('homeo_doctor.view_xray_billing_line_search').id,
            'target': 'current',
            'context': {
...
class ReportXRAYollection(models.AbstractModel):
    _name = 'report.homeo_doctor.xray_collection_report_template'
    _description = 'X-RAY Collection Report'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        records = self.env['xray.billing'].search(domain, order="bill_date asc")
        if doctor_id:
...
class xrayBillingLineWizard(models.TransientModel):
    _name = "xray.billing.line.wizard"
    _description = "xray Billing Line Wizard"

    wizard_id = fields.Many2one('xray.collection.report.wizard', string="Wizard")
    ...
    def unlink_line(self):
        ...
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'xray.collection.report.wizard',
