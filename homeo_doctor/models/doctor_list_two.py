...
class DoctorWiseCollectionListTwo(models.TransientModel):
    _name = 'doctor.wise.collection.list.two'
    ...
    doctor_id = fields.Many2one('doctor.profile', string="Doctor", domain=[('list_two', '=', True)])
    ...
    line_ids = fields.One2many(
        'doctor.wise.collection.lines.two',
        'wizard_ids',
    ...
    def action_view_report(self):
        ...
        if new_lines_vals:
            self.env['doctor.wise.collection.lines.two'].create(new_lines_vals)

        return {
            'type': 'ir.actions.act_window',
            'res_model': 'doctor.wise.collection.list.two',
            'view_mode': 'form',
            'res_id': self.id,
            'target': 'new',
    ...
    def action_print_excel(self):
        ...
        """.format(
            doc_filter="AND dp.id = %s" if doctor_id else "AND dp.list_two = TRUE"
        )
...
class DoctorWiseCollectionLineTwo(models.TransientModel):
    _name = 'doctor.wise.collection.lines.two'
    _description = 'Doctor Wise Collection Line'
    _order = 'date asc'

    wizard_ids = fields.Many2one(
        'doctor.wise.collection.list.two',
        string="Wizard",
        ondelete="cascade"
    )
...
class PatientRegistrationFee(models.Model):
    ...
    def unlink(self):
        ...
        DoctorLine = self.env['doctor.wise.collection.lines.two']
        for rec in self:
...
#
#         if vals_list:
#             self.env['doctor.wise.collection.lines.two'].create(vals_list)
#
#         return {
#             'type': 'ir.actions.act_window',
#             'res_model': 'doctor.wise.collection.list.two',
...
#         """.format(
#             doc_filter="AND pr.doc_name = %(doctor_id)s" if doctor_id else "AND dp.list_two = TRUE"
#         )
#
...
#         num_format    = workbook.add_format({'border': 1, 'num_format': '#,##0.00'})
#         bold_center   = workbook.add_format({'bold': True, 'align': 'center'})
#         grand_format  = workbook.add_format({'bold': True, 'border': 1})
#         grand_num_fmt = workbook.add_format({'bold': True, 'border': 1, 'num_format': '#,##0.00'})
#
#         sheet.merge_range('A1:E1', 'SANTHWANA HOSPITAL', title_format)
#         sheet.merge_range('A2:E2', 'N.C.C Road, Ambalamukku, Trivandrum-695005', bold_center)
#         sheet.merge_range('A3:E3', 'Phone: 0471-2432121, 4223131', bold_center)
#         sheet.merge_range('A4:E4', 'OP CONSULTATION REPORT', title_format)
#
...
#         sheet.merge_range('A5:E5', f"From: {date_from_str}    To: {date_to_str}", bold_center)
#
#         headers = [
#             'Sl No',
#             'Doctor Name',
...
#             sheet.write(row, 0, sl,           cell_format)
#             sheet.write(row, 1, res['name'],  cell_format)
#             sheet.write(row, 2, res['with'],  cell_format)
#             sheet.write(row, 3, res['without'], cell_format)
#             sheet.write(row, 4, res['total'], num_format)
#
#             grand_with    += res['with']
...
#
#         # Grand Total row
#         sheet.write(row, 0, '',            grand_format)
#         sheet.write(row, 1, 'Grand Total', grand_format)
#         sheet.write(row, 2, grand_with,    grand_format)
#         sheet.write(row, 3, grand_without, grand_format)
#         sheet.write(row, 4, grand_total,   grand_num_fmt)
#
#         sheet.set_column('A:A', 8)
...
#
#
# class PatientRegistrationFeeTwo(models.Model):
#     _inherit = 'patient.registration.fee'
#
#     def unlink(self):
#         DoctorLine = self.env['doctor.wise.collection.lines.two']
...
#                 lines.write({'registration_fee': False})
#                 self.env.cr.flush()
#         return super(PatientRegistrationFeeTwo, self).unlink()